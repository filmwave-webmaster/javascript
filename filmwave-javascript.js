...
function navigateStandaloneTrack(direction) {
  ...
  // Stop and destroy current audio completely
  ...
  // Create NEW audio element (starts from 0)
...
function setupMasterPlayerControls() {
  ...
  const navigateTrack = (direction) => {
    if (g.allWavesurfers.length > 0 && g.currentWavesurfer) {
      ...
      // Use standalone navigation when not on music page
...
function setupWaveformHandlers(wavesurfer, audioUrl, songData, cardElement, coverArtWrapper, songName) {
  ...
  const handlePlayPause = (e) => {
    ...
    if (g.currentWavesurfer && g.currentWavesurfer !== wavesurfer) {
      ...
      if (wasPlaying) {
        playStandaloneSong(audioUrl, songData, wavesurfer, cardElement, null, true);
      } else {
        g.currentWavesurfer = wavesurfer;
        g.currentSongData = songData;
        g.hasActiveSong = true;
...
function displaySongs(songs) {
  ...
  // Initialize waveforms directly after displaying songs
  initializeWaveforms();
}
...
async function initMusicPage() {
  ...
  if (isMusicPage) {
    ...
    // Display songs and initialize waveforms
    displaySongs(songs);
    ...
    setTimeout(() => {
      ...
      // Call updateMasterPlayerVisibility to ensure correct state
      updateMasterPlayerVisibility();
    }, 1000); // Give plenty of time for DOM to settle
  } else {
    // For non-music pages
    initMasterPlayer();
    updateMasterPlayerVisibility();
...
if (typeof barba !== 'undefined') {
  ...
  if (isMusicPage) {
    ...
    g.allWavesurfers.forEach(ws => {
      try {
        // Stop any pending loads to prevent AbortError
        ws.stop();
        ws.unAll();
        ws.destroy();
      } catch (error) {
